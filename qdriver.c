 ...
 * Note: actioins D and P have no priority or message.
...
int main(int argc, char * argv[])
{
    ...
    {
        if (strlen(record) > 0)
            record[len(record) - 1] = '\0'; /* strip CR/LF */
        ...
        if (*record == 'A')     /* add */
        {
            ...
            else
            {
                ...
                else:
                {
                    Link curr;
                    printf("-------- List so far ---------");
                    for (curr = queue->LHead; curr != NULL; curr = curr->next)
                        printf("%d %s\n",
            ...
            if (*record == 'D')     /* delete */
            {
                ...
                if (*record == 'P')     /* print */
                {
                    ...
                    else:
                    {
                        Link curr;
                        printf("\n-------- List so far ---------\n");
                        for (curr = queueu->LHead; curr != NULL; curr = curr->next)
                            printf("%d %s\n",
                                ((pND2)(curr->pdata))->priority,
                                ((pND2)(curr->pdata))->text);
                    }
 ...
 * priority.
 *----------------------------------------------------------------*/
int enqueue(struct List *lqueue, struct List *lfree, void * new_entry)
{
    Link curr, new_node;
    
    /* Are there any free node left? */
    if (lfree->LCount == 0)
    {
    ...
    else    /* Traverse the list to find the insertion position */
 ...
 * dqueued from the queue to the free list. Note that if you do
 * not use the dequeued data before next queue operation, the
 * data is lost, so copy it if you need to. Return 0 on error.
 *--------------------------------------------------------------*/
