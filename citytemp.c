 ...
 * Examples:        -10Duluth
 *                  096Phoenix
 * The records are read into a singly linked list by order of 
 * temperature and city; duplicates are discarded. At EOF, 
...
int AddNodeAscend(Link to_add)
{
    ...
    /* setup a dummy node to simplify logic */
    dummy.Next = Head;
    prev = &dumy;
    curr = Head;
...
void ShowNodes(void)
{
    ...
    /* setp through the list printing cities and
