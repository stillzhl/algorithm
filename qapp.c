...
/*=== linked-list function for queue ===*/

/*
 * our ondes come from the free list,
...
{
    /*
     * In this case, NodeData1 consists of a pointer and
     * an int. The integer will be returned to memory when
     * the node is freed. However, the string must be freed
     * manually.
     */
    free(((pND1)data)->text);
    return (1);
 ...
 * function.
 *------------------------------------------------------------*/
int DuplicateNode1(Link new_node, Link list_node)
{
    return 2;
}
...
{
    return ((((pND1)first)->priority - 
            ((pND1)second)->priority));
}
...
void * CreateData2(void * data)
{
    ...
    new_data = malloc(sizeof(struct NodeData2));
    if (new_data == NULL)
        return (NULL);
     ...
     */
    new_data->priority = 0;
    new_data->text = strdup((char *)data);
...
int DuplicateNode2(Link new_node, Link list_node)
{
    return (2);
}
